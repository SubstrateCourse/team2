...
decl_storage! {
	...
	trait Store for Module<T: Trait> as TemplateModule {
		...
		AccountToProofs get(fn proofs_by_account): map hasher(blake2_128_concat) T::AccountId =>  Vec<(Vec<u8>, T::BlockNumber, Vec<u8>)>;
	}
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn create_claim(origin, claim: Vec<u8>, note: Vec<u8>) -> dispatch::DispatchResult {
			...
			let mut list;
			if !AccountToProofs::<T>::contains_key(&sender) {
				list = Vec::<(Vec<u8>, T::BlockNumber, Vec<u8>)>::new();
			} else {
				list = AccountToProofs::<T>::get(sender.clone());
			}

			list.push((claim.clone(), system::Module::<T>::block_number(), note.clone()));

			AccountToProofs::<T>::insert(sender.clone(), list);
