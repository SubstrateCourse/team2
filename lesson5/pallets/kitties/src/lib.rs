...
impl<T: Trait> Module<T> {
	...
	fn insert_kitty(owner: T::AccountId, kitty_id: u32, kitty: Kitty) {
		...
		let owner_kitties_id = <OwnedKittiesCount<T>>::get(&owner);
		OwnedKitties::<T>::insert((owner.clone(), owner_kitties_id), kitty_id);
		OwnedKittiesCount::<T>::insert(owner.clone(), owner_kitties_id + 1);
	}
